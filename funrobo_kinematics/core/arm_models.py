...
class ScaraRobotTemplate():
    ...
    def calc_forward_kinematics(
        ...
        """
        Compute the forward kinematics of the SCARA robot.

        Returns the end-effector pose and a list of **individual link transforms**.

        Args:
            joint_values (list[float]): Joint variables. Units depend on `radians` for revolute joints.
            radians (bool, optional): If False, revolute joint inputs are assumed to be degrees and
                will be converted to radians. Defaults to True.

        Returns:
            tuple:
                - EndEffector: End-effector pose container.
                - list[np.ndarray]: List of 4x4 individual link transforms (length = num_dof).
        """
        ee = ut.EndEffector()
        Hlist = []
    ...
    def calc_inverse_kinematics(
        ...
        """
        Compute an inverse kinematics solution for the SCARA robot.

        Args:
            ee (EndEffector): Desired end-effector pose.
            joint_values (list[float]): Initial or previous joint values (for continuity/selection).
            soln (int, optional): Solution branch index if multiple solutions exist. Defaults to 0.

        Returns:
            list[float]: Joint values that achieve the desired pose.
        """
        new_joint_values = joint_values.copy()
        return new_joint_values
    ...
    def calc_velocity_kinematics(
        ...
        """
        Update joint values based on desired end-effector velocity.

        Args:
            joint_values (list[float]): Current joint values.
            vel (list[float]): Desired end-effector linear velocity.
            dt (float, optional): Integration time step in seconds. Defaults to 0.02.

        Returns:
            list[float]: Updated joint values after one time step.
        """
        new_joint_values = joint_values.copy()
        return new_joint_values
  

    def calc_robot_points(self, joint_values: list, H, radians=True):
        """
        Compute key robot points and end-effector pose for visualization.

        Args:
            joint_values (list[float]): Joint values. Units depend on `radians`.
            H (np.ndarray | None): Array of individual 4x4 transforms with shape (num_dof, 4, 4).
        ...
        self.points[2] = H[0]@self.points[0]
        self.points[3] = self.points[2] + np.array([0, 0, self.l3, 1])
        self.points[4] = H[0]@H[1]@self.points[0] + np.array([0, 0, self.l5, 1])
        self.points[5] = H[0]@H[1]@self.points[0]
        self.points[6] = H[0]@H[1]@H[2]@self.points[0]

        self.EE_axes = H[0]@H[1]@H[2]@np.array([0.075, 0.075, 0.075, 1])
        H_ee = H[0]@H[1]@H[2]
...
class FiveDOFRobotTemplate():
    ...
    def calc_forward_kinematics(
        ...
        Args:
            joint_values (list[float]): Joint angles. Units depend on `radians`.
            radians (bool, optional): If False, joint angles are assumed to be degrees and
                will be converted to radians. Defaults to True.
    ...
    def calc_inverse_kinematics(
        ...
        Args:
            ee (EndEffector): Desired end-effector pose.
            joint_values (list[float]): Initial or previous joint angles (for continuity/selection).
            soln (int, optional): Solution branch index if multiple solutions exist. Defaults to 0.

        Returns:
            list[float]: Joint angles that achieve the desired pose.
        """
        new_joint_values = joint_values.copy()
        return new_joint_values
    ...
    def calc_velocity_kinematics(
        ...
        """
        Update joint angles based on desired end-effector velocity.

        Args:
            joint_values (list[float]): Current joint angles in radians.
            vel (list[float]): Desired end-effector velocity.
            dt (float, optional): Integration time step in seconds. Defaults to 0.02.

        Returns:
            list[float]: Updated joint angles after one time step.
        """
        new_joint_values = joint_values.copy()
